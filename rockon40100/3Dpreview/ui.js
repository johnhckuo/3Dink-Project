...
//選單能叫連接，拿掉只會讀到gcode的數據
  /*$('.gcode_examples a').on('click', function(event) {

    GCodeImporter.importPath($(this).attr('href'), onGCodeLoaded);
    return false;
  })*/
	$('.gcode_examples input').on('change',function(event) {
        var file = document.getElementById('fileToUpload').files[0];
		
      })

	$('.gcode_examples ').on('click',function(event) {
        var fd = new FormData();
	...
	function uploadComplete(evt) {
        ...
        GCodeImporter.importPath(uploadpossion.FilePath, onGCodeLoaded);
    return false;
